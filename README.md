...
`git`安装完成后需要在Windows命令行(cmd)中输入以下命令测试一下:(不知道怎么打开命令行?[点击这里](pages/cmd简要介绍.md))
```
git -v
```
...
首先创建远程仓库,进入自己账户的主页,点击绿色的`NEW`以创建新的仓库:
![](assets/README/file-20251124160538382.png)

如图(仅做参考)配置:

![](assets/README/file-20251124160659002.png)

确认无误后点击`Create repository`,完成远程仓库的创建

回到github网站的用户主界面,找到创建好的私人仓库:

![](assets/README/file-20251124160931413.png)
1. 点击醒目的绿色的`<> Code`键,选择`SSH`并复制
...
然后打开该路径下的cmd([不知道怎么打开的点击这里](pages/cmd简要介绍.md)),**输入以下两命令中的一个**(请看完):

```
git clone git@github.com:"你的用户名"/"你的仓库名".git
```

该命令的效果是在你选择的本地文件夹中创建一个新的文件夹作为仓库,你只需要输入`git clone`,后面的部分直接就是刚刚复制的内容,粘贴即可
...
搜索`git`,下载并启用,启用后设置中左下方会多出一栏`git`,点击进入:

![](assets/README/file-20251124160415282.png)
